...
#' @param minNug minimum nugget tolerance to avoid singular covariance matrix
...
#'
krige_sp <- function(formula, data, newdata, model, errorVar, minNug = 1e-7, computeVar = FALSE) {
  ...
  if(length(nug_ind)) {
    nug <- pmax(model$psill[nug_ind], minNug)
    model <- model[-nug_ind, ]
  }

  if(model$psill == 0) model <- gstat::vgm(minNug, "Nug", 0)
